...
> **이 연구는 종료되었습니다.** 45개 DOE 실험(8,850 에피소드) 결과, 반사적 의사결정 도메인에서 RAG 기반 전략 검색은 측정 가능한 효과가 전혀 없음을 확인했습니다. 오직 움직임만이 성능을 결정하며(Cohen's d = 1.408), 비율-시간 보상 법칙으로 인해 모든 전술적 최적화는 무의미합니다.

## 연구 결과

산업용 DOE 방법론을 적용한 45개 체계적 실험을 통해 다음을 발견했습니다:

1. **움직임이 유일한 성능 결정 요소** — 좌우로 이동하는 에이전트가 정지 에이전트보다 킬 수 65% 우세 (d=1.408, p<0.001). 다른 어떤 요소도 이에 비할 수 없음.

2. **RAG 가설 반증** — 3번의 독립 실험(N=630)에서 L2 검색이 하드코딩된 규칙 대비 효과 제로. 문서 품질의 효과도 문자 그대로 0 (d=0.000).

3. **비율-시간 보상 법칙** — 공격 빈도를 높이면 생존 시간이 비례 감소하여 총 킬 수 불변 (4배 공격 범위에서 CV < 6%). 전술적 최적화가 근본적으로 불가능한 이유.

4. **환경이 에이전트 설계를 압도** — 시나리오 난이도가 성능 분산의 77% 설명 (η²=0.769). 에이전트 아키텍처 파라미터는 합산 5% 미만.

5. **진화는 효과적이나 움직임에 한정** — 5세대 진화로 +54% 성능 향상 달성. 하지만 최적화된 파라미터는 회전-이동 비율 — 역시 움직임.

6. **액션 스페이스 최적점** — 이산 액션 5~7개가 최적. 3개는 회피 불가, 15개는 유해 액션 포함.

7. **RAG 시스템에 대한 시사점** — 이 결과는 반사적 도메인에 한정됩니다. 지식 집약적 환경(기업 에이전트, 코드 생성)에서 RAG의 가치는 근본적으로 다릅니다 — 자연어 추론을 통한 검색 컨텍스트 활용은 수치 스코어링이 포착할 수 없는 의사결정을 가능하게 합니다.

**논문**: [NeurIPS 초안](research/paper/PAPER_DRAFT.tex) | **데이터**: [CSV 내보내기](research/data/) | **검증**: `make verify`
