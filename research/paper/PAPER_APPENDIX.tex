...
\author{
  ...
  \textit{Movement Is All You Need: How 29 Systematic Experiments} \\
...
\midrule
\textbf{Total} & --- & \textbf{29} & --- & \textbf{5,010} & --- & --- & --- & --- \\
\bottomrule
\end{longtable}
\end{small}
...
\begin{itemize}
\item \textbf{Total Experiments}: 29 DOEs
\item \textbf{Total Episodes}: 5,010 episodes
\item \textbf{Average Episodes per DOE}: 172.8 episodes
\item \textbf{Phase Distribution}:
  ...
  \end{itemize}
\item \textbf{Significant Results ($p < 0.05$)}: 18 of 29 (62\%)
\item \textbf{Null Results ($p \geq 0.05$)}: 11 of 29 (38\%)
\item \textbf{Largest Effect Size}: $\eta^2=0.720$ (doom\_skill in DOE-023)
...
\section{Key Findings Catalogue (Top 20)}
\label{app:findings}
...
\section{Variance Decomposition Summary}
\label{app:variance}

This table shows the relative importance of different factors in explaining performance variance across all experiments. Values represent percentage of total variance explained ($\eta^2 \times 100\%$).
...
\midrule
\textbf{doom\_skill (difficulty)} & 0.39--0.72 & 0.720 & \textbf{72\%} & 022, 023, 024 & Dominant environmental factor; enemy capability determines outcome far more than agent design \\
\textbf{Movement presence} & 0.22--0.33 & 0.332 & \textbf{33\%} & 029 & Sole agent-controlled factor with substantial effect; all other architectural choices negligible \\
...
This table presents detailed evidence for the rate-time compensation law discovered in DOE-027 and validated in DOE-028/029. The law states: \textbf{within the movement class, kill rate increases compensate for survival time decreases such that total kills remain invariant}.
...
Let:
\begin{itemize}
\item $k_r(t)$ = kill rate at time $t$ (kills/minute)
\item $T$ = survival time (seconds)
\item $K$ = total kills = $(k_r \times T) / 60$
\end{itemize}

\textbf{Compensation Law} (within movers):
\begin{align*}
...
\textbf{Empirical Elasticity} (DOE-027):
\begin{itemize}
\item Attack ratio increases 9$\times$ (10\% $\rightarrow$ 90\%)
\item Kill rate increases 1.54$\times$ (32.2 $\rightarrow$ 49.7 kr/min): \textbf{+54\%}
\item Survival time decreases 0.53$\times$ (30.1 $\rightarrow$ 16.0s): \textbf{--47\%}
\item Elasticity ratio: 54\% / 47\% = \textbf{1.15} (near-perfect compensation)
\end{itemize}

\textbf{Boundary Condition} (DOE-029):
\begin{itemize}
\item Compensation active: movement present
\item Compensation fails: movement absent (10.4 vs 17.2 kills, 65\% gap)
\end{itemize}
...
\begin{enumerate}
\item \textbf{Attack ratio tuning is irrelevant} for total kills (within movers)
\item \textbf{Movement presence is critical} for accessing compensation mechanism
\item \textbf{Multi-objective optimization} (rate $\times$ time) more robust than single-objective (rate OR time)
\item \textbf{Survival-first strategies fail} because they sacrifice rate without gaining sufficient time (F-065)
\item \textbf{Burst structure irrelevant} (F-077) because compensation operates at rate-time level, not tactical sequence level
\end{enumerate}
...
\begin{itemize}
\item \textbf{Raw Episode Data}: 5,010 episodes $\times$ $\sim$50 metrics = 250,500 data points
\item \textbf{ANOVA Tables}: 29 experiments $\times$ 3 response variables = 87 tables
\item \textbf{Diagnostic Plots}: 29 experiments $\times$ 3 diagnostics = 87 plots
...
\subsection{Proposed Experiments (DOE-030+)}

\textbf{DOE-030}: Movement Gradient (10 levels)
\begin{itemize}
\item Factor: Movement probability (0\%, 10\%, 20\%, ..., 90\%, 100\%)
\item Purpose: Identify exact boundary where compensation fails
\item Expected: Threshold effect around 10--20\% movement probability
\end{itemize}

\textbf{DOE-031}: Prompt Engineering Factorial
\begin{itemize}
\item Factors: Prompt length $\times$ Context depth $\times$ Update frequency
\item Purpose: Give L2 RAG one final chance with optimized prompts
\item Expected: Still null, but necessary to rule out prompt issues
\end{itemize}

\textbf{DOE-032}: Multi-Scenario Generalization
\begin{itemize}
\item Factor: 5 scenarios $\times$ 3 difficulties
\item Purpose: Test if findings generalize beyond defend\_the\_line
\item Expected: Movement class effect generalizes, others scenario-specific
\end{itemize}
...
\item \textbf{Sequential Testing}: Adaptive sample size based on interim results
\item \textbf{Meta-Analysis}: Formal synthesis across all 29 experiments
\item \textbf{Publication Bias Check}: Assess whether null results were appropriately reported (answer: yes, 38\% null rate)
\end{enumerate}
...
\begin{itemize}
\item \textbf{Total Compute}: $\sim$5,010 episodes $\times$ 60s = 84 hours game time
\item \textbf{Hardware}: Single workstation (AMD Ryzen 9, NVIDIA RTX 3090)
\item \textbf{Energy Estimate}: $\sim$10 kWh (modest for ML research)
\end{itemize}
...
%% =====================================================================
%% SECTIONS I--O: Content moved from main body during page compression
%% Cross-references point back to corresponding main paper sections.
%% =====================================================================

\section{Detailed Phase 0--1 Results (DOE-001--020)}
\label{app:phase01}

This section expands the compressed Phase 0--1 narrative in Section~4.1 of the main paper, providing full experimental detail for the 20 experiments that preceded hypothesis falsification.

\subsection{Infrastructure Validation (DOE-001--004)}
...
\subsection{Scenario Selection (DOE-007--008)}
...
\subsection{Agent Parameter Null Results (DOE-009)}
...
\subsection{Strategy Landscape (DOE-010--020)}
...
\label{app:strategy-ranking}

This table was referenced in Section~4.1 of the main paper as part of the Phase 0--1 results. It presents the complete performance ranking from the best-of-breed tournament (DOE-020).
...
When a policy increases its attack ratio (the proportion of ticks allocated to the ATTACK action), two countervailing effects occur simultaneously: (i) more shots are fired per unit time, increasing $r$; and (ii) fewer ticks are available for strafing, increasing damage intake and decreasing $s$. We observe empirically that the marginal gain in $r$ is exactly offset by the marginal loss in $s$.

DOE-027 showed that increasing attack ratio from 0.2 to 0.8:
\begin{itemize}
\item Raises kill rate from 36.5/min to 42.0/min ($F(6,203)=3.736$, $p=0.0015$, $\eta_p^2=0.099$)
\item Reduces survival from 26.2s to 21.3s (linear trend: $-7.77$ s per unit ratio, $p=0.016$)
\item Total kills remain statistically invariant ($F(6,203)=0.617$, $p=0.717$, $\eta_p^2=0.018$)
\end{itemize}

\subsection{Movement Class Constants}
...
\subsection{Conservation Tightness}
...
\section{Rate-Time Compensation Numerical Evidence}
\label{app:rtc-tables}

This section presents the full numerical tables referenced in Section~4.3 of the main paper.

\begin{table}[h]
\centering
\caption{Rate-time compensation across attack ratios (DOE-027, $n=30$ per condition). Kill rate increases and survival decreases with attack ratio, but total kills remain invariant ($F(6,203)=0.617$, $p=0.717$). The compensation ratio $rs/60k$ stays within 3\% of unity.}
\label{tab:app-compensation}
\small
\begin{tabular}{ccccc}
\toprule
...
\textbf{Observation.} As attack ratio increases from 0.2 to 0.8 ($4\times$ range), kill rate increases 15\% (36.47 to 41.99 kr/min) while survival decreases 19\% (26.22 to 21.29s). The resulting total kills vary by only 16\% (14.70 to 17.40), and the compensation ratio $rs/60k$ remains within [0.996, 1.021], confirming near-perfect rate-time compensation.


\section{Information-Theoretic Analysis}
\label{app:info-theory}

This section presents the information-theoretic perspective on rate-time compensation, referenced in Section~5 of the main paper.
...
\subsection{Equalization Forces}

Three equalization forces create the performance convergence zone:
...
\subsection{5-Action Space Extension}

In the expanded 5-action space $\{$TURN\_LEFT, TURN\_RIGHT, MOVE\_LEFT, MOVE\_RIGHT, ATTACK$\}$, the maximum entropy increases to:
...
The information-theoretic perspective provides a precise explanation for the tactical invariance finding (F-077): the action-to-outcome channel has near-zero capacity for strategic information within a movement class. All strategy selection effort operates on a channel that transmits only 0.082 bits of strategically relevant information. Optimizing strategy selection is informationally equivalent to optimizing a nearly constant function.


\section{Extended Variance Decomposition}
\label{app:ext-variance}

This section expands the variance decomposition summary in Section~5 of the main paper and Appendix~C, providing the full narrative analysis.
...
\centering
\small
\caption{Complete variance decomposition across all 29 experiments. Values represent proportion of total variance explained ($\eta^2$) from the primary source experiment.}
\label{tab:app-ext-variance}
\begin{tabular}{@{}lcccl@{}}
\toprule
Factor & $\eta^2$ & Source DOE & $n$ & Interpretation \\
\midrule
doom\_skill (difficulty) & 0.720 & DOE-023 & 360 & 72\% of variance; dominant \\
Movement presence & 0.332 & DOE-029 & 120 & 33\% of variance; sole agent lever \\
Architecture (L0/L1/L2) & 0.127--0.267 & DOE-008/010 & 150/90 & Driven by L0-only deficit \\
Strategy type (within movers) & $<$0.03 & DOE-027/028 & 360 & $<$3\% of variance \\
L2 RAG configuration & 0.001--0.009 & DOE-022/024/026 & 630 & $<$1\% of variance \\
Agent parameters & 0.002--0.017 & DOE-009 & 270 & $<$1\% of variance \\
Action composition & 0.008--0.027 & DOE-018/028 & 270 & $<$3\% of variance \\
Attack ratio & 0.008--0.018 & DOE-015/027 & 330 & $<$2\% of variance \\
Residual & $\sim$0.20 & All & 5,010 & Irreducible stochasticity \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Key conclusions.} Environment settings (doom\_skill) and the binary movement choice together explain over 80\% of all performance variance. The entire agent architecture stack above L0 heuristics---including RAG retrieval, parameterized decision weights, and tactical action selection---contributes less than 5\% of total variance. This finding fundamentally challenges the premise that architectural complexity is the primary lever for performance improvement in this domain.

The hierarchy of importance is: \textbf{Environment} ($\eta^2 = 0.720$) $\gg$ \textbf{Movement} ($\eta^2 = 0.332$) $\gg$ \textbf{Architecture} ($\eta^2 \approx 0.15$, driven by L0-only) $\gg$ \textbf{Strategy} ($\eta^2 < 0.03$) $\gg$ \textbf{Parameters/RAG} ($\eta^2 < 0.01$).

Traditional RL approaches focus on policy optimization (equivalent to our ``strategy'' and ``parameters'' factors), which explain only $\sim$4\% of variance. Our systematic DOE reveals that environmental factors (72\%) and basic architectural decisions (33\%) dominate, suggesting prior work optimized negligible factors while ignoring dominant sources of variance.


\section{Extended Discussion: The Value of Negative Results}
\label{app:negative-results}

This section expands the discussion of negative results referenced in Section~6 of the main paper.
...
\subsection{Publication Bias and Null Result Quality}

The ML research community exhibits strong publication bias toward positive results \cite{henderson2018deep}. Our work demonstrates that rigorously established null results carry substantial scientific value:

\begin{itemize}
\item \textbf{F-070 (RAG falsification)}: Three independent null results with cumulative $N=630$, spanning two action spaces and three retrieval implementations. This is not a single underpowered non-replication but a convergent pattern of evidence.
\item \textbf{F-077 (tactical invariance)}: Confirmed across 12 conditions spanning attack ratios and temporal patterns, with near-perfect rate-time compensation ratios (0.980--1.003).
\item \textbf{F-015 (parameter irrelevance)}: $3^2$ factorial with $N=270$ episodes, all factors non-significant with adequate power.
\end{itemize}

Each of these null results is established with statistical rigor equivalent to or exceeding the typical positive result in game AI research: large sample sizes, proper ANOVA with diagnostics, effect size reporting, and independent replication. The 38\% null rate across our 29 experiments (11 of 29) provides a natural check against publication bias---we report failures as rigorously as successes.
