...
Figure~\ref{fig:architecture} illustrates the four-level architecture. The key experimental finding---that L2 strategy retrieval provides no measurable benefit (Section~\ref{sec:falsification})---challenges the core design assumption.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/figure1_architecture.pdf}
\caption{Four-level hierarchical agent architecture. L0 (hardcoded rules, $<1$ms) provides a deterministic behavioral floor. L1 (episode cache, $<10$ms) stores periodic action patterns. L2 (strategy retrieval, $<100$ms) performs $k$NN vector search over strategy documents generated by L3 retrospection. L3 operates offline between episodes. The red marker on L2 indicates the core falsification result: RAG-based strategy retrieval produces no measurable performance benefit ($p > 0.39$ across three independent tests).}
